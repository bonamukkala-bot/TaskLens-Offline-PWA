...
import {
  AlertCircle, Archive, ArrowLeft, Bell, BellRing, Camera, Check, CheckCircle2,
  ChevronDown, ChevronRight, CircleHelp, Clock3, FileText, Filter, Headphones,
  ImagePlus, LoaderCircle, Mic, Moon, Pencil, Plus, Save, Settings, Sparkles,
  Sun, Trash2, Upload, Volume2, WifiOff, X, Zap,
} from 'lucide-react';
...
function LensProvider({ children }: { children: ReactNode }) {
  ...
  // On-open & Foreground reminder check
  useEffect(() => {
    const runReminderCheck = async () => {
      try {
        const settings = await getSettings();
        if (!settings.due_date_alerts_enabled) return;
        if (getNotificationPermission() !== 'granted') return;

        const storedTasks = await listTasks();
        const classification = classifyTasks(storedTasks, new Date());
        const urgentTasks = [...classification.overdue, ...classification.dueToday];
        if (urgentTasks.length === 0) return;

        // Check last_reminder_check_at to avoid repeating on rapid tab toggles within 5 minutes
        const now = Date.now();
        if (settings.last_reminder_check_at) {
          const lastCheck = Date.parse(settings.last_reminder_check_at);
          if (!isNaN(lastCheck) && now - lastCheck < 5 * 60 * 1000) {
            return;
          }
        }

        const summary = getReminderSummary(classification);
        const total = urgentTasks.length;

        sendBrowserNotification(`TaskLens: ${total} ${total === 1 ? 'task needs' : 'tasks need'} attention`, {
          body: summary.notificationBody,
          tag: 'tasklens-reminder-check',
        });

        await updateSettings({ last_reminder_check_at: new Date().toISOString() });
      } catch (err) {
        console.warn('Reminder check failed', err);
      }
    };

    const timer = window.setTimeout(() => void runReminderCheck(), 600);
...
function DashboardPage() {
  ...
  const [collapsed, setCollapsed] = useState<Partial<Record<TaskSectionKey, boolean>>>({});

  // Classify tasks against exact current datetime
  const classification = useMemo(() => classifyTasks(tasks, new Date()), [tasks]);
  const overdueCount = classification.overdue.length;
  const dueTodayCount = classification.dueToday.length;
  const hasUrgent = overdueCount > 0 || dueTodayCount > 0;
  const reminderSummary = useMemo(() => getReminderSummary(classification), [classification]);

  // Filter tasks based on search, status, priority, and urgent toggle
  const visible = useMemo(() => {
    const urgentIds = new Set([...classification.overdue, ...classification.dueToday].map((t) => t.id));
    return tasks
      .filter((task) => {
        if (urgentOnly && !urgentIds.has(task.id)) return false;
  ...
  }, [tasks, urgentOnly, filter, priorityFilter, query, dueSort, classification]);

  // Group filtered tasks into collapsible sections
  const sections = useMemo(() => groupTasksIntoSections(visible, new Date()), [visible]);

  const toggleSection = (key: TaskSectionKey) => {
  ...
  const sectionConfigs: {
    ...
    { key: 'overdue', label: 'Overdue', items: sections.overdue, isOverdue: true, badgeStyle: 'bg-destructive/15 text-destructive border-destructive/30' },
    { key: 'today', label: 'Today', items: sections.today, badgeStyle: 'bg-amber-500/15 text-amber-600 dark:text-amber-300 border-amber-500/30' },
    { key: 'tomorrow', label: 'Tomorrow', items: sections.tomorrow, badgeStyle: 'bg-primary/15 text-primary border-primary/25' },
    { key: 'thisWeek', label: 'This week', items: sections.thisWeek, badgeStyle: 'bg-muted text-muted-foreground border-border' },
    { key: 'later', label: 'Later', items: sections.later, badgeStyle: 'bg-muted text-muted-foreground border-border' },
    { key: 'noDate', label: 'No date', items: sections.noDate, badgeStyle: 'bg-muted text-muted-foreground border-border' },
    { key: 'completed', label: 'Completed', items: sections.completed, badgeStyle: 'bg-muted/80 text-muted-foreground border-border' },
  ];

  const activeSections = sectionConfigs.filter((s) => s.items.length > 0);
  ...
  return (
    <section data-testid="page-dashboard">
      <Header
        ...
        title="A clear head starts here."
        subtitle={openCount ? `${openCount} open ${openCount === 1 ? 'task' : 'tasks'} waiting for you.` : 'Capture the thought before it gets away.'}
        action={
          <button
            type="button"
            onClick={() => openComposer()}
            data-testid="button-add-task"
            className="hidden items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/15 transition-transform hover:-translate-y-0.5 md:flex"
          >
      ...
      {/* Dismissible Reminder Banner for Overdue / Due-today with exact time awareness */}
      {hasUrgent && !bannerDismissed && (
        <div
          role="status"
          aria-live="polite"
          data-testid="banner-reminders"
          className="fade-up mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-primary/30 bg-gradient-to-r from-card via-accent/35 to-card p-4 shadow-sm backdrop-blur-md"
        >
          <div className="flex flex-wrap items-center gap-2.5 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {overdueCount > 0 && (
                <span
                  data-testid="badge-banner-overdue"
                  className="inline-flex items-center gap-1.5 rounded-lg border border-destructive/30 bg-destructive/15 px-2.5 py-1 text-xs font-semibold text-destructive"
                >
                  <AlertCircle size={13} />
              ...
              {dueTodayCount > 0 && (
                <span
                  data-testid="badge-banner-today"
                  className="inline-flex items-center gap-1.5 rounded-lg border border-amber-500/30 bg-amber-500/15 px-2.5 py-1 text-xs font-semibold text-amber-600 dark:text-amber-300"
                >
                  <Clock3 size={13} />
            ...
            <span className="text-xs font-medium text-foreground/90 truncate max-w-[280px] sm:max-w-none">
              {reminderSummary.bannerHeadline}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              onClick={() => {
                ...
                if (!urgentOnly) setFilter('open');
              }}
              data-testid="button-banner-filter"
      ...
      <div className="mb-6 flex flex-col gap-3">
        <label className="relative flex-1">
          <Filter size={16} className="absolute left-3.5 top-3.5 text-muted-foreground" />
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            data-testid="input-search-tasks"
            placeholder="Find a task"
            className="h-11 w-full rounded-xl border border-input bg-card pl-10 pr-4 text-sm outline-none transition-colors placeholder:text-muted-foreground/70 focus:border-primary focus:ring-2 focus:ring-primary/15"
          />
        </label>
        <div className="flex flex-wrap gap-2">
          <div className="flex rounded-xl border border-border bg-card p-1" role="group" aria-label="Task visibility">
            {(['all', 'open', 'done'] as const).map((item) => (
              <button
                ...
                onClick={() => {
                  setFilter(item);
                  if (item === 'done' && urgentOnly) setUrgentOnly(false);
                }}
                data-testid={`button-filter-${item}`}
                className={`rounded-lg px-3.5 py-2 text-xs font-semibold capitalize transition-colors ${
                  filter === item && !urgentOnly ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
          ...
          <select
            value={dueSort}
            onChange={(event) => setDueSort(event.target.value as typeof dueSort)}
          ...
          {urgentOnly && (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-xl border border-primary/30 bg-primary/10 px-3 text-xs font-semibold text-primary"
            >
              Urgent only <X size={13} />
            </button>
          )}
        </div>
      </div>
      ...
      {tasks.length === 0 ? (
        <EmptyDashboard onAdd={() => openComposer()} />
      ) : visible.length === 0 ? (
        <EmptyFilter
          onReset={() => {
            setQuery('');
        ...
        <div className="space-y-6" data-testid="list-tasks">
            ...
            return (
              <div
                ...
                className={`rounded-2xl transition-all ${
                  section.isOverdue
                    ? 'border border-destructive/25 bg-destructive/5 p-3.5'
                    : ''
                }`}
              >
                <button
                  ...
                  className="mb-3 flex w-full items-center justify-between gap-2 text-left group"
                >
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-xs font-bold uppercase tracking-wider ${
                        section.isOverdue ? 'text-destructive' : 'text-foreground/80'
                      }`}
                    >
                      {section.label}
                    </span>
                    <span
                      className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-bold ${section.badgeStyle}`}
                    >
                      {section.items.length}
                    </span>
                  </div>
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg text-muted-foreground transition-colors group-hover:bg-muted group-hover:text-foreground">
                    <ChevronDown
                      size={15}
      ...
      {tasks.length > 0 && (
        <div className="mt-7 flex items-center justify-between border-t border-border/70 pt-4 text-xs text-muted-foreground">
...
function TaskCard({ task, index, onToggle, onEdit, onDelete }: { task: Task; index: number; onToggle: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  const formattedDue = formatTaskDueLabel(task.due, new Date());
  ...
  return <article onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} className={`fade-up delay-${Math.min(index + 1, 3)} group rounded-2xl border bg-card p-4 transition-colors ${task.completed ? 'border-border/60 opacity-70' : 'border-card-border hover:border-primary/35'}`} data-testid={`card-task-${task.id}`}>
    <div className="flex gap-3.5">
      <button type="button" onClick={onToggle} aria-label={task.completed ? 'Mark task open' : 'Complete task'} data-testid={`button-toggle-task-${task.id}`} className={`mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full border transition-colors ${task.completed ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground/50 hover:border-primary'}`}>
        {task.completed && <Check size={13} strokeWidth={3} />}
      </button>
      <div className="min-w-0 flex-1">
        <button type="button" onClick={onEdit} data-testid={`button-edit-task-${task.id}`} className="text-left">
          <h3 className={`text-[15px] font-semibold leading-snug ${task.completed ? 'text-muted-foreground line-through' : ''}`}>{task.title}</h3>
          {task.details && <p className="mt-1.5 line-clamp-2 text-sm leading-relaxed text-muted-foreground">{task.details}</p>}
        </button>
        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px] font-medium text-muted-foreground">
          <span className="inline-flex items-center gap-1 rounded-md bg-muted px-2 py-1" title={`${task.source} source`}>
            {task.source === 'voice' ? <Mic size={12} /> : task.source === 'camera' ? <Camera size={12} /> : <Pencil size={12} />}
            {task.source}
          </span>
          {formattedDue && (
          ...
          <span className={`rounded-md px-2 py-1 capitalize ${task.priority === 'high' ? 'bg-destructive/15 text-destructive' : task.priority === 'medium' ? 'bg-amber-500/15 text-amber-600 dark:text-amber-300' : 'bg-muted text-muted-foreground'}`}>
            {task.priority}
          </span>
        </div>
      </div>
      <div className="flex shrink-0 items-start gap-1 opacity-100 sm:opacity-0 sm:transition-opacity sm:group-hover:opacity-100">
        <button type="button" onClick={onEdit} aria-label="Edit task" data-testid={`button-edit-icon-${task.id}`} className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground">
          <Pencil size={15} />
        </button>
        <button type="button" onClick={onDelete} aria-label="Delete task" data-testid={`button-delete-task-${task.id}`} className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive">
          <Trash2 size={15} />
        </button>
      </div>
    </div>
...
function ComposerSheet({ task, close }: { task?: Task; close: () => void }) {
  ...
  const { date: initialDate, time: initialTime } = splitDueDateTime(task?.due);
  ...
  const submit = (event: FormEvent) => {
    ...
    const combinedDue = combineDueDateTime(dueDate, dueTime);
    if (task) updateTask(task.id, { title: title.trim(), details: details.trim(), due: combinedDue, priority });
    else createTask({ title: title.trim(), details: details.trim(), due: combinedDue, priority, source: 'manual' });
    close();
  };
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-[hsl(var(--background)/.72)] p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-label={task ? 'Edit task' : 'Add task'}>
    <div className="fade-up w-full max-w-[520px] rounded-t-[28px] border border-border bg-card p-5 shadow-2xl sm:rounded-[28px] sm:p-7">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[.18em] text-primary">{task ? 'Refine task' : 'Quick capture'}</p>
          <h2 className="mt-1 text-xl font-semibold">{task ? 'Edit task' : 'Add a task'}</h2>
        </div>
        <button type="button" onClick={close} data-testid="button-close-composer" className="rounded-xl p-2 text-muted-foreground hover:bg-muted">
          <X size={19} />
        </button>
      </div>
      <form onSubmit={submit} className="space-y-4">
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Task title</span>
          <input
            autoFocus
            value={title}
            ...
            className="h-12 w-full rounded-xl border border-input bg-background px-3.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
          />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Notes <span className="font-normal">(optional)</span></span>
          <textarea
            value={details}
            ...
            rows={3}
            className="w-full resize-none rounded-xl border border-input bg-background px-3.5 py-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
          />
        </label>
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="block">
            <span className="mb-1.5 block text-xs font-semibold text-muted-foreground">
              Time <span className="font-normal text-[11px]">(optional)</span>
            </span>
            <input
              type="time"
              value={dueTime}
        ...
        <fieldset className="block">
          <legend className="mb-1.5 block text-xs font-semibold text-muted-foreground">Priority</legend>
          <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-input bg-background p-1">
            {(['low', 'medium', 'high'] as const).map((option) => (
              <button
                key={option}
                type="button"
                onClick={() => setPriority(option)}
                className={`min-h-9 rounded-lg px-1 text-[11px] font-semibold capitalize ${priority === option ? 'bg-accent text-accent-foreground' : 'text-muted-foreground'}`}
              >
                {option}
              </button>
            ))}
        ...
        {error && <p className="text-sm text-destructive" data-testid="status-composer-error">{error}</p>}
        <button type="submit" data-testid="button-save-task" className="mt-2 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary text-sm font-semibold text-primary-foreground">
          <Save size={17} />{task ? 'Save changes' : 'Save task'}
        </button>
      </form>
    </div>
  </div>;
...
function CapturePage({ mode }: { mode: 'voice' | 'camera' }) {
  ...
  const sample = () => {
    ...
    const startedAt = performance.now();
    window.setTimeout(() => void processText(isVoice ? "Submit the DBMS assignment tomorrow at 6pm, it's urgent." : 'Review the handwritten project notes next Friday at 9am.', isVoice ? 'voice' : 'camera', startedAt), 500);
  };
...
function ReviewPage() {
  ...
  const { date: initialDate, time: initialTime } = splitDueDateTime(draft?.due);
  const [dueDate, setDueDate] = useState(initialDate);
  const [dueTime, setDueTime] = useState(initialTime);
  ...
  const save = (event: FormEvent) => {
    ...
    const combinedDue = combineDueDateTime(dueDate, dueTime);
    const finalDraft = { ...draft, due: combinedDue };
    createTask(finalDraft);

    // Part 2: Demo flourish notification (foreground-only, ~8s later for high-priority task)
    if (finalDraft.priority === 'high' && getNotificationPermission() === 'granted') {
      scheduleForegroundReminder(finalDraft.title.trim(), 8000);
    }

    sessionStorage.removeItem('tasklens.draft');
  ...
  return <section data-testid="page-review">
    <Link href="/" data-testid="link-back-review" className="mb-7 inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground">
      <ArrowLeft size={17} />Back to tasks
    </Link>
    <Header eyebrow="One last look" title="Make it yours." subtitle="TaskLens made a best guess. Tune the wording, then save it when it feels right." />
    {!draft ? (
      <div className="rounded-[24px] border border-dashed border-border bg-card p-12 text-center" data-testid="empty-review">
        <FileText size={26} className="mx-auto mb-3 text-muted-foreground" />
        <p className="font-semibold">Nothing is waiting for review</p>
        <p className="mt-1 text-sm text-muted-foreground">Capture something with voice or camera to see it here.</p>
        <Link href="/voice" data-testid="link-review-capture" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-primary">
          Start a capture <ChevronRight size={15} />
        </Link>
      </div>
    ) : (
      <form onSubmit={save} className="space-y-4" data-testid="form-review">
        <div className="rounded-[24px] border border-primary/25 bg-accent/35 p-5">
          <div className="mb-4 flex items-center gap-2 text-xs font-semibold text-primary"><Sparkles size={15} />Proposed task</div>
          <label className="block">
            <span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Title</span>
            <input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} data-testid="input-review-title" className="h-12 w-full rounded-xl border border-input bg-card px-3.5 text-sm outline-none focus:border-primary" />
          </label>
          <label className="mt-4 block">
            <span className="mb-1.5 block text-xs font-semibold text-muted-foreground">What the AI heard/read</span>
            <textarea value={draft.details} onChange={(event) => setDraft({ ...draft, details: event.target.value })} data-testid="input-review-details" rows={3} className="w-full resize-none rounded-xl border border-input bg-card px-3.5 py-3 text-sm outline-none focus:border-primary" />
          </label>
        </div>
        <details className="rounded-2xl border border-border bg-card px-4 py-3">
          <summary className="cursor-pointer text-sm font-semibold">Raw capture</summary>
          <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">{draft.rawText || draft.details}</p>
        </details>
        <div className="grid grid-cols-2 gap-3">
          <label>
            <span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Due date</span>
            <input
              ...
              onChange={(event) => {
                ...
                const combined = combineDueDateTime(event.target.value, dueTime);
                setDraft((current) => current ? { ...current, due: combined } : current);
              }}
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => {
                ...
                const combined = combineDueDateTime(dueDate, event.target.value);
                setDraft((current) => current ? { ...current, due: combined } : current);
              }}
              data-testid="input-review-time"
              className="h-11 w-full rounded-xl border border-input bg-card px-3 text-sm outline-none focus:border-primary"
            />
          </label>
        </div>
        <fieldset>
          <legend className="mb-1.5 block text-xs font-semibold text-muted-foreground">Priority</legend>
          <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-input bg-card p-1">
            {(['low', 'medium', 'high'] as const).map((option) => (
              <button key={option} type="button" onClick={() => setDraft({ ...draft, priority: option })} className={`min-h-9 rounded-lg px-1 text-[11px] font-semibold capitalize ${draft.priority === option ? 'bg-accent text-accent-foreground' : 'text-muted-foreground'}`}>
                {option}
              </button>
            ))}
          </div>
        </fieldset>
        {draft.processingTimeMs && <p className="text-xs text-muted-foreground">Processed on-device in {(draft.processingTimeMs / 1000).toFixed(1)}s</p>}
        <div className="flex flex-col gap-2 pt-2 sm:flex-row">
          <button type="submit" disabled={saved} data-testid="button-review-save" className="flex h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-primary text-sm font-semibold text-primary-foreground">
            {saved ? <Check size={17} /> : <Save size={17} />}{saved ? 'Saved' : 'Save task'}
          </button>
          <button type="button" onClick={discard} data-testid="button-review-discard" className="h-12 rounded-xl border border-border px-5 text-sm font-semibold text-muted-foreground hover:bg-muted">
            Discard
          </button>
        </div>
      </form>
...
function SettingsPage() {
  ...
  const handleToggleAlerts = async (checked: boolean) => {
    if (checked) {
      ...
      if (currentPerm === 'granted') {
        const updated = await updateSettings({ due_date_alerts_enabled: true });
        setSettings(updated);
      ...
      const updated = await updateSettings({ due_date_alerts_enabled: false });
      setSettings(updated);
  ...
  const runSelfTest = async () => {
    const startedAt = performance.now();
    const result = await extractTasks('Submit the project notes tomorrow at 6pm, it is urgent.');
  ...
  return <section data-testid="page-settings"><Header eyebrow="Control room" title="Quiet settings." subtitle="TaskLens is yours. Keep it local, choose your look, and check what is ready." /><div className="space-y-4">
    ...
    {/* Due-date alerts on-device reminder toggle */}
    <div className="rounded-[24px] border border-border bg-card p-5 md:p-6" data-testid="card-due-date-alerts">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3.5">
          <div className="mt-0.5 grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-primary/15 text-primary">
            <Bell size={18} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <label htmlFor="due-date-alerts-toggle" className="font-semibold text-base cursor-pointer">
                Due-date alerts
              </label>
              <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${
                settings?.due_date_alerts_enabled && notifPermission === 'granted'
                  ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30'
                  : 'bg-muted text-muted-foreground'
              }`}>
                {settings?.due_date_alerts_enabled && notifPermission === 'granted' ? 'Enabled' : 'Disabled'}
              </span>
            </div>
            <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
              Notify about overdue and due-today tasks when opening the app.
            </p>
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground/90 font-medium">
              Reminders are on-device only: alerts only fire when the app is open or brought to the foreground, since TaskLens never contacts a server.
            </p>
          </div>
        </div>
        <div className="flex items-center pt-1">
          <SwitchToggle
            id="due-date-alerts-toggle"
            aria-label="Due-date alerts"
            data-testid="switch-due-date-alerts"
            checked={Boolean(settings?.due_date_alerts_enabled && notifPermission === 'granted')}
      ...
      {showPermissionPrompt && (
        <div className="mt-4 rounded-2xl border border-primary/30 bg-accent/40 p-4 fade-up" data-testid="prompt-notification-permission">
          <p className="text-sm font-semibold">Enable on-device browser alerts?</p>
          <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
            TaskLens will show browser notifications for overdue and due-today tasks whenever you launch or open the app. No data ever leaves your device.
          </p>
          <div className="mt-3 flex gap-2">
            <button
              ...
              className="rounded-xl bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow-sm"
            >
              Allow notifications
            </button>
            <button
      ...
      {notifPermission === 'denied' && (
        <div className="mt-3 rounded-xl border border-destructive/20 bg-destructive/10 p-3 text-xs text-destructive">
          Browser notification permission is blocked. To receive due-date alerts, enable notifications in your browser site permissions.
        </div>
      )}
      {notifPermission === 'unsupported' && (
        <div className="mt-3 rounded-xl bg-muted p-3 text-xs text-muted-foreground">
          Browser notifications are not supported in this environment. The in-app overdue banner will continue to work.
        </div>
      )}
    </div>
